...
export function Menu({onClose, initialView}: MenuProps): React.ReactElement {
  ...
  if (view === 'pref-workflow-preset') {
    const items: SelectItem[] = [
      ...
      {id: 'assistant', label: '◇ Assistant mode'},
    ];
    const modeToIndex = {normal: 0, specialist: 1, artist: 2, explorer: 3, assistant: 4};
    ...
    return (
      ...
      >
        <SelectList
          ...
          onConfirm={i => {
            const presets = ['normal', 'specialist', 'artist', 'explorer', 'assistant'] as const;
  ...
  if (view === 'pref-context-editing') {
    const items: SelectItem[] = [{id: 'on', label: '✓ Slim down the context'}, {
      id: 'off',
